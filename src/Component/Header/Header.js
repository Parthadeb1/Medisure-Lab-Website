...
const Header = () => {
  ...
  return (
    ...
    <Navbar  collapseOnSelect expand="lg" bg="dark" variant="dark" sticky="top">
      <Container>
     <Link to="/home"> <Navbar.Brand className="fs-3 fw-bold">Medi<span className="text-danger">Health</span> </Navbar.Brand></Link>
      <Navbar.Toggle />
    <Navbar.Collapse className="justify-content-end">
        ...
        {user.email ?
        <button onClick={logOut} type="button" className="btn btn-danger fw-normal">SignOut</button>:
        <Nav.Link className="fs-6 fw-bold text-white"as={Link} to="/signin">SignIn</Nav.Link>
        }

      <Navbar.Text className="p-2"><span className="text-info">{element1} </span> 
       <span>{user && <span>{user.displayName}</span> || <span>{user?.name}</span> }</span> 
      </Navbar.Text>
    </Navbar.Collapse>
      </Container>
    </Navbar>
